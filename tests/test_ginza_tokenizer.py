...
def test_ginza_returns_natural_bunsetu_slots_without_punctuation():
...
def test_ginza_failure_uses_safe_lossless_fallback(monkeypatch):
    import tokenizer

    monkeypatch.setattr(tokenizer, "_document", lambda text: (_ for _ in ()).throw(RuntimeError("boom")))
    sentence = "私は、fallback でも開けます。"
    analysis = tokenizer.analyze_sentence(sentence)
    assert analysis["source"] == "fallback"
    assert tokenizer.reconstruct_sentence(analysis["chunks"], analysis["structure"]) == sentence
    assert fixed_texts(analysis) == ["、", " ", "。"]
