...
function renderReport() { const r = state.report; if (!r) return route('reports', {replace:true}); view.innerHTML = `<section class="page"><div class="page-head"><div><h1>本轮练习报告</h1><p>${formatDate(r.completed_at || r.created_at)}</p></div><div><button class="btn outline" data-action="toggle-wrong">只看错误</button> <button class="btn outline" data-action="home">返回首页</button> <button class="btn primary" data-action="retry-report">重新练习本轮</button></div></div><div class="report-summary"><div class="card stat-card"><strong>${r.total}</strong>总句数</div><div class="card stat-card"><strong>${r.correct}</strong>正确</div><div class="card stat-card"><strong>${r.wrong}</strong>错误</div><div class="card stat-card"><strong>${r.skipped}</strong>跳过</div><div class="card stat-card"><strong>${r.accuracy}%</strong>正确率</div></div><div id="report-items">${reportItems(r.items)}</div></section>`; setChrome(); }
